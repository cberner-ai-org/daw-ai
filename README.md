...
Every Gemini session is retained locally with request/response JSON, graph state, metadata, and rendered WAV artifacts. By default sessions live under `users/<cookie>/gemini-sessions/` beside each user's project. The `users/` directory is created in the directory containing `DAW_AI_PROJECT_PATH`, or in the working directory when no project path is configured. `DAW_AI_GEMINI_SESSION_DIR` overrides the base location and keeps each user's sessions in a separate child directory. The Debug tab lists the latest sessions by timestamp.
