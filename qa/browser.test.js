  ...
  try {
    ...
    try {
      await waitFor(
        async () => evaluate(cdp, realStreamingPage.sessionId, `(() => {
          ...
          const spectrumLevels = [...document.querySelectorAll('.track-spectrum i')]
            .map((bar) => Number.parseFloat(bar.style.getPropertyValue('--spectrum-level')) || 0);
          window.__realStreamingStatus = {
            audioState: document.documentElement.dataset.audioState,
            currentTime: window.__realStreamingMedia.currentTime,
            displayedSeconds,
            maximumSpectrumLevel: Math.max(0, ...spectrumLevels),
            spectrumState: document.querySelector('.track-spectrum')?.dataset.state,
          };
          return window.__realStreamingStatus.currentTime >= 16.1 &&
            Math.abs(displayedSeconds - window.__realStreamingStatus.currentTime) < 0.15 &&
            window.__realStreamingStatus.maximumSpectrumLevel > 0.01;
        })()`),
      ...
      throw new Error(`${error.message}: ${JSON.stringify(status)}`);
    }
    ...
    const offlineRender = await evaluate(
      ...
      `(async () => {
        ...
        const duration = 16;
        const lastByte = 44 + duration * 48000 * 4 - 1;
    ...
    const audioEngineSource = fs.readFileSync(path.join(root, "web", "audio-engine.js"), "utf8");
    const clientSource = appSource + audioEngineSource;
    const clientAudioBoundary = {
      apiNoStore: appSource.includes('cache: "no-store"'),
      backendSpectrumTimeline:
        audioEngineSource.includes("/api/track-spectrum/") &&
        audioEngineSource.includes("this.media.currentTime"),
      frontendWorkers: audioEngineSource.includes("new Worker("),
      offlineContext: clientSource.includes("OfflineAudioContext"),
      oscillator: clientSource.includes("createOscillator"),
      backendEndpoint: audioEngineSource.includes("/api/audio-stream/"),
      trackSpectrumEndpoint: audioEngineSource.includes("/api/track-spectrum/"),
      timestampedSpectrum:
        audioEngineSource.includes("frameDuration") && audioEngineSource.includes("projectTime"),
      idleWarmupOnly: audioEngineSource
        .slice(
          audioEngineSource.indexOf("warmOpeningSpectrum()"),
          audioEngineSource.indexOf("\n      updateAnalyzerState("),
        )
        .includes("this.isActive"),
      mediaClock: audioEngineSource.includes("media.currentTime"),
      performanceClock: audioEngineSource.includes("performance.now"),
      prefetch: audioEngineSource.includes("beginPrefetch"),
      reusableMedia: (audioEngineSource.match(/new Audio\(\)/g) || []).length,
      boundedRetry:
        audioEngineSource.includes("AUDIO_RETRY_DELAYS_MS") &&
        audioEngineSource.includes("retryPlayback"),
    };
    assert.deepEqual(
      clientAudioBoundary,
      {
